 ...
 * @api public
 */
function GenericNote(accessToken, type) {
  if (!(this instanceof GenericNote)) {
    return new GenericNote(accessToken, type);
  }
...
GenericNote.OAuthClient = function(key, secret, type) {
  ...
  switch (type) {
    case GenericNote.notetype.EVERNOTE:
      this.authClient = new Evernote.Client({
        consumerKey: key,
...
OAProto = GenericNote.OAuthClient.prototype;
